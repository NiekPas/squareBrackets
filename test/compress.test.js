...
describe('compress function test', () => {
  ...
  it('should compress an array with empty strings', () => {
    ...
    const compressedArr = squareBrackets.compress(arr);
    expect(compressedArr).to.deep.equal([4, 15, 8, 42, 23, 16]);
  });
  ...
  it('should not remove nonempty arrays', () => {
    ...
    const compressedArr = squareBrackets.compress(arr);
    expect(compressedArr).to.deep.equal(arr);
  });
  ...
  it('should not remove nonempty objects', () => {
    ...
    const compressedArr = squareBrackets.compress(arr);
    expect(compressedArr).to.deep.equal([4, {student: true, id: 8439102, name: 'John Daniels'}, 15, 8, {name: 'Frank', trait: 'Frank'}, 42, 23, 16]);
  });
